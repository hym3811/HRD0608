...
<form name=form>
	<br>
	<h3 style="font-size:2em;text-align:center;">교과목 수정</h3>
	<br>
	<table border=1 style="width:50%;margin:0 auto;">
		<%
			...
			try{
				...
				if(rs.next()){
					...
					if(Integer.parseInt(end_hour)<1000){
						end_hour="0"+end_hour;
					}
				}
			}catch(Exception e){
		...
		<tr>
			...
			<td><input type=text name=subject_name style="width:100%;height:40px;font-size:1.2em;font-weight:bold;" value="<%=subject_name%>"></td>
		</tr>
		<tr>
			<th style="height:40px;width:30%;font-size:1.1em;">학점</th>
			<td><input type=number name=credit style="width:100%;height:40px;font-size:1.2em;font-weight:bold;" value="<%=credit%>"></td>
		</tr>
		<tr>
					...
					<%
						...
						try{
							...
							while(rs.next()){
								...
								name=rs.getString(2);
					%>
					<option value="<%=idx%>" <%=lecturer.equals(idx) ? "selected" : "" %>>[<%=idx%>]<%=name %></option>
					<%
							}
						}catch(Exception e){
							e.printStackTrace();
		...
		<tr>
			...
			<td>
				...
				%>
					<label style="width:100%;height:40px;font-size:1.1em;font-weight:bold;margin:0 1.2%;"><input type=radio name=week value="<%=i%>" <%=Integer.toString(i+1).equals(week) ? "checked" : "" %>><%=d %></label>
				<%
					}
				%>
		...
		<tr>
			...
			<td><input type=number name=start_hour style="width:100%;height:40px;font-size:1.2em;font-weight:bold;" value="<%=start_hour%>"></td>
		</tr>
		<tr>
			<th style="height:40px;width:30%;font-size:1.1em;">종료시간</th>
			<td><input type=number name=end_hour style="width:100%;height:40px;font-size:1.2em;font-weight:bold;" value="<%=end_hour%>"></td>
		</tr>
		<tr>
			<td colspan=2 style="height:40px;text-align:center;">
				...
				<input type=button value="수정" onclick=add() style="background-color:black;color:white;width:70px;height:40px;margin:0 2%;font-size:1.2em;font-weight:bold;">
			</td>
		</tr>
	</table>
</form>
...
function add(){
	...
	if(doc.id.value==""){
		...
		doc.action="updateSubjectProcess.jsp";
		doc.submit();
	}
}
function sel(){
